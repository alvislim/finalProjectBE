...
module.exports = {
    productEntry: async (req, res) => {
        try {
            ...
            const newProduct = new Product({ url, price, desiredPrice, img, email, name, condolidatedPrice: [{ prices: desiredPrice }] })
            ...
            for (let i = 0; i < updatedRecommended.length; i++) {
                let productId = await Product.findByIdAndUpdate(savedProduct._id,
                    {
                        $push: {
                            recommendedProducts: { prodName: updatedRecommended[i].name, prodPrice: updatedRecommended[i].price }
                        }
                    })
            }


            httpResponse(res, 201, true, 'Items has been sucessfully added for price tracking', savedProduct)
        } catch (err) {
            console.log(err)
